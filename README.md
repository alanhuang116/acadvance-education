...
| | 值 | 位置 |
|---|---|---|
| **显示** | `admissions@acadvances.com` | 链接文字、表单提示 |
...
2. 如果访客直接手打显示地址发信，那封信会退回——`admissions@acadvances.com` 目前并不存在。

**两个问题都靠同一件事解决：去域名注册商后台开一条邮件转发规则**，`admissions@acadvances.com → alanhuang116@gmail.com`（Cloudflare、阿里云、GoDaddy 都免费提供）。开通后，把 [about.html](about.html) 表单上的 `data-mailto` 也改成品牌邮箱，Gmail 地址就彻底不出现在站上了。
